...
public:
    ...
    inline uint32_t avail_len() const { return _total_len - _used_len; }

    inline bool empty() const { return _used_len == 0; }

    inline void append(const char* log_line, uint32_t len){
        if (avail_len() < len)
            return ;
...
#define LOG_MEM_SET(mem_lmt) \
    do{ \
        ...
        rlog::ring_log::_one_buff_len = mem_lmt; \
    } while (0)

#define LOG_INIT(log_dir, prog_name, level) \
    do{ \
        rlog::ring_log::ins()->init_path(log_dir, prog_name, level); \
        pthread_t tid; \
        pthread_create(&tid, NULL, rlog::be_thdo, NULL); \
        pthread_detach(tid); \
    } while (0)


#define TRACELOG(fmt, args...) \
	do{ \
		if (rlog::ring_log::ins()->get_level() >= rlog::TRACE){ \
			rlog::ring_log::ins()->try_append("[TRACE]", "%s:%d: " fmt "\n", \
...
#define LOG_TRACE(fmt, args...) \
    do{ \
        if (rlog::ring_log::ins()->get_level() >= rlog::TRACE){ \
            rlog::ring_log::ins()->try_append("[TRACE]", "[%u]%s:%d(%s): " fmt "\n", \
...
#define DEBUGLOG(fmt, args...) \
	do{ \
		if (rlog::ring_log::ins()->get_level() >= rlog::DEBUG){ \
			rlog::ring_log::ins()->try_append("[DEBUG]", "%s:%d: " fmt "\n", \
			__FILE__, __LINE__, ##args); \
		} \
	} while (0)
...
#define LOG_DEBUG(fmt, args...) \
    do{ \
        if (rlog::ring_log::ins()->get_level() >= rlog::DEBUG){ \
            rlog::ring_log::ins()->try_append("[DEBUG]", "[%u]%s:%d(%s): " fmt "\n", \
                    gettid(), __FILE__, __LINE__, __FUNCTION__, ##args); \
        } \
    } while (0)
...
#define INFOLOG(fmt, args...) \
	do{ \
		if (rlog::ring_log::ins()->get_level() >= rlog::INFO){ \
			rlog::ring_log::ins()->try_append("[INFO]", "%s:%d: " fmt "\n", \
			__FILE__, __LINE__, ##args); \
		} \
	} while (0)
...
#define LOG_INFO(fmt, args...) \
    do{ \
        if (rlog::ring_log::ins()->get_level() >= rlog::INFO){ \
            rlog::ring_log::ins()->try_append("[INFO]", "[%u]%s:%d(%s): " fmt "\n", \
                    gettid(), __FILE__, __LINE__, __FUNCTION__, ##args); \
        } \
    } while (0)


#define NORMALLOG(fmt, args...) \
	do{ \
		if (rlog::ring_log::ins()->get_level() >= rlog::INFO){ \
			rlog::ring_log::ins()->try_append("[INFO]", "%s:%d: " fmt "\n", \
			__FILE__, __LINE__, ##args); \
...
#define WARNLOG(fmt, args...) \
	do{ \
		if (rlog::ring_log::ins()->get_level() >= rlog::WARN){ \
			rlog::ring_log::ins()->try_append("[WARN]", "%s:%d: " fmt "\n", \
			__FILE__, __LINE__, ##args); \
		} \
	} while (0)
...
#define LOG_WARN(fmt, args...) \
    do{ \
        if (rlog::ring_log::ins()->get_level() >= rlog::WARN){ \
            rlog::ring_log::ins()->try_append("[WARN]", "[%u]%s:%d(%s): " fmt "\n", \
                    gettid(), __FILE__, __LINE__, __FUNCTION__, ##args); \
        } \
    } while (0)
...
#define ERRORLOG(fmt, args...) \
	do{ \
		if (rlog::ring_log::ins()->get_level() >= rlog::ERROR){ \
			rlog::ring_log::ins()->try_append("[ERROR]", "%s:%d: " fmt "\n", \
			__FILE__, __LINE__, ##args); \
		} \
	} while (0)
...
#define LOG_ERROR(fmt, args...) \
    do{ \
        if (rlog::ring_log::ins()->get_level() >= rlog::ERROR){ \
            rlog::ring_log::ins()->try_append("[ERROR]", "[%u]%s:%d(%s): " fmt "\n", \
                gettid(), __FILE__, __LINE__, __FUNCTION__, ##args); \
        } \
    } while (0)


#define FATALLOG(fmt, args...) \
	do{ \
		rlog::ring_log::ins()->try_append("[FATAL]", "%s:%d: " fmt "\n", \
		__FILE__, __LINE__, ##args); \
	} while (0)

#define LOG_FATAL(fmt, args...) \
    do{ \
        rlog::ring_log::ins()->try_append("[FATAL]", "[%u]%s:%d(%s): " fmt "\n", \
            gettid(), __FILE__, __LINE__, __FUNCTION__, ##args); \
    } while (0)
