...
public:
	...
	ConnectionPtr back_connptr(int32 frontfd) {
		...
		boost::unordered_map<int32, int32>::iterator it = _front2back_fd_map.find(frontfd);
	...
	ConnectionPtr front_connptr(int32 backfd) {
		ConnectionPtr connptr;
		safe::ReadLockGuard g(_fd_map_mtx);
		boost::unordered_map<int32, int32>::iterator it = _back2front_fd_map.find(backfd);
	...
	void unassociate_by_front(int32 frontfd) {
		...
		boost::unordered_map<int32, int32>::iterator it = _front2back_fd_map.find(frontfd);
		if (it != _front2back_fd_map.end()) {
			boost::unordered_map<int32, int32>::iterator it2 = _back2front_fd_map.find(it->second);
	...
	void unassociate_by_back(int32 backfd) {
		...
		boost::unordered_map<int32, int32>::iterator it = _back2front_fd_map.find(backfd);
		if (it != _back2front_fd_map.end()){
			boost::unordered_map<int32, int32>::iterator it2 = _front2back_fd_map.find(it->second);
...
private:
	...
	boost::unordered_map<int32, int32> _front2back_fd_map;
	boost::unordered_map<int32, int32> _back2front_fd_map;

	DISALLOW_COPY_AND_ASSIGN(Connmap);
};

typedef boost::shared_ptr<Connmap> ConnmapPtr;
