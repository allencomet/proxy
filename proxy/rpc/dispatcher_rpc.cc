...
namespace proxy {
	namespace rpc {
		...
		void Dispatcher::init(int32 listenfd) {
			...
			_front_threadptr.reset(new safe::Thread(boost::bind(&Dispatcher::run_front, this)));
			_back_threadptr.reset(new safe::Thread(boost::bind(&Dispatcher::run_back, this)));
		}
		...
		void Dispatcher::handle_write_event_back(int32 fd) {
			...
			if (connptr) {
				...
				for (std::deque<StreamBufPtr>::iterator it = write_queue.begin();
					...
					LOG << "epoll has written a packet to back,information: userid[" << tmp->ueser_id << "],bodylen[" << tmp->body_len
