...
namespace proxy {
    namespace rpc {
        ...
        void RequestHandler::create_new_back_process(const request &req) {
            ...
            if (jsonReader.parse(req.content.data(), jsonValue))
                userid = jsonValue["user_id"].asString();//从json串中解析出用户ID
            if (userid.empty()) {
        ...
        //将前端请求包转发到后端服务器
        void
        RequestHandler::front_request_to_back(ConnectionPtr backptr, const request &req, const std::string &userid) {
