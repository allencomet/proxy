    ...
    namespace ipc {
        ...
        void RequestHandler::handle_request(request &req) {
            ...
            if (BACK_EXIT == req.ipc_pack.msg_type) {
                ...
                LOG << "received client[" << connptr->addr() << "] request information: function_number["
                    << req.ipc_pack.func_no <<
                    "],flow_no[" << req.ipc_pack.flow_no << "],msg_type[" << req.ipc_pack.msg_type << "],content["
